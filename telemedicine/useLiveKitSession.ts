...
import type { SessionState, SessionParticipantInfo, LiveKitTokenResponse } from "@/types/telemedicine.types";
...
export function useLiveKitSession({
  ...
  const [room] = useState(() => new Room({
    adaptiveStream: true,          // Auto-adjust kualitas video
    dynacast: true,                // Hemat bandwidth
    ...
    videoCaptureDefaults: {
      resolution: {                // HD tapi hemat bandwidth
        width: 1280,
        height: 720,
        frameRate: 24,
      },
  ...
  const mapParticipant = useCallback(
    (p: RemoteParticipant | LocalParticipant): SessionParticipantInfo => {
      const meta = (() => {
        try { return JSON.parse(p.metadata ?? "{}"); }
        catch { return {}; }
      })();
      return {
        identity: p.identity,
        ...
        networkQuality: p.connectionQuality === ConnectionQuality.Excellent ? 5
          : p.connectionQuality === ConnectionQuality.Good ? 3
          : p.connectionQuality === ConnectionQuality.Poor ? 1
          : 0,
      };
    },
    []
  ...
  useEffect(() => {
    const handleConnected = () => {
      ...
      timerRef.current = setInterval(() => {
        setSessionState((s) => ({ ...s, elapsedSeconds: s.elapsedSeconds + 1 }));
      }, 1000);
    };
  ...
  }, [room, refreshParticipants, mapParticipant, updateState, onSessionEnd, onParticipantJoined, onParticipantLeft]);
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      const { data }: { data: LiveKitTokenResponse } = await res.json();
      ...
      console.error("[useLiveKitSession] connect error:", err);
    }
