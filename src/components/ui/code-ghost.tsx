...
function colorize(line: string): React.ReactNode[] {
  ...
  while (rest.length > 0) {
    ...
    for (const [re, color] of rules) {
      ...
      if (m) {
        tokens.push(<span key={i++} style={{ color }}>{m[0]}</span>);
    ...
    if (!matched) {
      tokens.push(<span key={i++} style={{ color: "#3d3a39" }}>{rest[0]}</span>);
      rest = rest.slice(1);
    }
  }
  return tokens;
...
export function CodeGhost() {
  ...
  return (
    ...
    >
      <pre
        style={{
          fontFamily: 'ui-monospace, "Fira Code", "Geist Mono", monospace',
          fontSize: "12.5px",
          lineHeight: "1.8",
          userSelect: "none",
          textAlign: "left",
