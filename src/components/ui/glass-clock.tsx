...
export function GlassClock(): React.ReactElement {
  ...
  useEffect(() => {
    ...
    const updateHourAndMinuteHands = () => {
      const now = new Date();
      const localStr = now.toLocaleString('en-US', { timeZone: BERLIN_TIMEZONE });
      ...
      hourMinuteTimeoutRef.current = window.setTimeout(updateHourAndMinuteHands, Math.max(msUntilNextMinute, 0));
    };
      ...
      <div className="gc-container">
        <div className="gc-effect-wrapper">
          ...
          <div className="gc-face">
            ...
            <div className="gc-timezone" ref={timezoneRef}>WIB</div>
          </div>
        </div>
      </div>
    </div>
