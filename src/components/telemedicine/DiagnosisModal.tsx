...
export function DiagnosisModal({ open, appointment, onClose }: DiagnosisModalProps): React.JSX.Element | null {
  ...
  const handleIcdSearch = useCallback(async (query: string) => {
    ...
    try {
      ...
      if (res.ok) {
        const data = (await res.json()) as { ok?: boolean; results?: IcdSearchResult[]; data?: IcdSearchResult[] };
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div>
            <h2 style={{ color: "var(--text-main)", fontSize: 16, fontWeight: 600, margin: 0 }}>
              Input Diagnosis
            </h2>
            ...
            <div style={{ position: "relative" }}>
              ...
              >
                ...
                <input
                  ...
                  style={{ flex: 1, background: "none", border: "none", outline: "none", color: "var(--text-main)", fontSize: 13, padding: "10px 0" }}
                />
                {isSearching && (
                  <div
                    style={{
              ...
              {icdResults.length > 0 && (
                ...
                >
                  {icdResults.map((r) => (
                    ...
                    >
                      <span style={{ color: "var(--c-asesmen)", fontFamily: "monospace", fontSize: 12, fontWeight: 700 }}>
                        {r.code}
                      </span>
                      <span style={{ color: "var(--text-main)", fontSize: 13, marginLeft: 8 }}>
                        {r.description}
                      </span>
                      {r.pCareCode && (
        ...
        >
          <button
            onClick={onClose}
            style={{ color: "var(--text-muted)", background: "none", border: "none", cursor: "pointer", fontSize: 13, padding: "8px 16px" }}
          >
            Batal
          </button>
          <button
...
function FormField({
  ...
  return (
    <div>
      ...
      {children}
      {error && (
        <p style={{ display: "flex", alignItems: "center", gap: 4, color: "#f87171", fontSize: 11, marginTop: 4 }}>
