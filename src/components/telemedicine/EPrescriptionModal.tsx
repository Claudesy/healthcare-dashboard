...
export function EPrescriptionModal({ open, appointment, onClose }: EPrescriptionModalProps): React.JSX.Element | null {
          ...
          {items.map((item, index) => (
            ...
            >
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                <span style={{ color: "var(--text-muted)", fontSize: 12, fontWeight: 600 }}>Obat #{index + 1}</span>
                {items.length > 1 && (
                  <button
                ...
                <div>
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    <select
                      ...
                      onChange={(e) => { if (e.target.value) handleUpdateItem(index, "aturanMinum", e.target.value); }}
                    >
...
function FieldLabel({ children }: { children: React.ReactNode }): React.JSX.Element {
  return (
    <label style={{ color: "var(--text-muted)", fontSize: 11, fontWeight: 600, display: "block", marginBottom: 4 }}>
      {children}
    </label>
  );
}