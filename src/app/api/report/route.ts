  ...
  try {
    ...
    if (summary) {
      ...
      return NextResponse.json({
        source: "lb1-summary",
        summary: [
          { label: "Total Kunjungan", value: String(summary.totalKunjungan), unit: period },
          { label: "Rawat Jalan",     value: String(summary.rawatJalan),     unit: "pasien" },
          { label: "Rawat Inap",      value: String(summary.rawatInap),      unit: "pasien" },
          { label: "Rujukan",         value: String(summary.rujukan),        unit: "kasus" },
        ],
        rows: summary.unmappedDx.length > 0
          ...
          : [{ rm: "-", nama: "Semua diagnosis terpetakan", tanggal: period, diagnosis: "Generate LB1 untuk melihat data kunjungan", icd: "-", dokter: "LB1 Engine", status: "SELESAI" }],
      });
    }
    ...
    if (history.length > 0) {
      ...
      return NextResponse.json({
        source: "lb1-history",
        summary: [
          { label: "Total Kunjungan", value: String(h.validRows + h.invalidRows), unit: period },
          { label: "Rawat Jalan",     value: String(h.rawatJalan ?? h.validRows), unit: "pasien" },
          { label: "Rawat Inap",      value: String(h.rawatInap ?? 0), unit: "pasien" },
          { label: "Rujukan",         value: String(history.filter(x => x.status === "failed").length), unit: "kasus" },
        ],
        rows: history.slice(0, 8).map((entry, i) => ({
          ...
          diagnosis: entry.status === "success"
