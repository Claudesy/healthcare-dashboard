...
import { generateLiveKitToken, ensureLiveKitRoom, isLiveKitConfigured } from "@/lib/telemedicine/token";
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  if (!isLiveKitConfigured()) {
    return NextResponse.json<ApiResponse<null>>(
      { success: false, data: null, message: "LiveKit belum dikonfigurasi. Isi LIVEKIT_URL, LIVEKIT_API_KEY, dan LIVEKIT_API_SECRET.", timestamp: new Date().toISOString() },
      { status: 503 }
    );
  }
  ...
  if (!parsed.success) {
    return NextResponse.json<ApiResponse<null>>(
      { success: false, data: null, message: "Parameter tidak valid", timestamp: new Date().toISOString() },
      { status: 400 }
    );
  }
  ...
  if (appointment.status === "CANCELLED" || appointment.status === "NO_SHOW") {
    return NextResponse.json<ApiResponse<null>>(
      { success: false, data: null, message: "Appointment sudah dibatalkan", timestamp: new Date().toISOString() },
      { status: 400 }
    );
  }
  ...
  if (!hasAccess) {
    await createAuditLog({
      ...
      metadata: { role: session.role, requestedParticipantRole: participantRole },
    });
    return NextResponse.json<ApiResponse<null>>(
      { success: false, data: null, message: "Akses ditolak", timestamp: new Date().toISOString() },
      { status: 403 }
    );
  }
  ...
  const roomName =
    appointment.livekitRoomName ?? `pkm-tele-${appointmentId}`;
  ...
  await prisma.telemedicineParticipant.upsert({
    where: { sessionId_userId: { sessionId: dbSession.id, userId: session.username } },
    create: {
  ...
  if (appointment.status === "CONFIRMED" || appointment.status === "PENDING") {
    await prisma.telemedicineAppointment.update({
      where: { id: appointmentId },
      data: { status: "IN_PROGRESS", startedAt: appointment.startedAt ?? new Date() },
    });
  }
  ...
  const token = await generateLiveKitToken({
    ...
    participantRole,
    metadata: JSON.stringify({ role: participantRole, userId: session.username }),
  });
