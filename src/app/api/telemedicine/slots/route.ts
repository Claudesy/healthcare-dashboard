...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  if (!doctorId || !date) {
    return NextResponse.json<ApiResponse<null>>(
      { success: false, data: null, message: "Parameter doctorId dan date wajib diisi", timestamp: new Date().toISOString() },
      { status: 400 }
    );
  }
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) {
    return NextResponse.json<ApiResponse<null>>(
      { success: false, data: null, message: "Format date harus YYYY-MM-DD", timestamp: new Date().toISOString() },
      { status: 400 }
    );
  }
