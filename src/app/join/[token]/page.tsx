...
import { Video, Mic, MicOff, VideoOff, Phone, AlertCircle, Loader2 } from "lucide-react";
...
export default function PatientJoinPage(): React.JSX.Element {
  ...
  const handleJoin = useCallback(async () => {
    ...
    try {
      ...
      const data = await res.json() as { success: boolean; data?: { token: string; roomName: string; serverUrl: string }; message?: string };
  ...
  const scheduledDate = info ? new Date(info.scheduledAt).toLocaleString("id-ID", {
  ...
  if (state === "loading") {
    return (
      <div style={fullCenter}>
        ...
        <p style={{ color: "rgba(255,255,255,0.5)", marginTop: 16, fontSize: 14 }}>Memuat informasi konsultasi...</p>
      </div>
    );
  }
  ...
  if (state === "error") {
    return (
      <div style={fullCenter}>
        ...
        <p style={{ color: "rgba(255,255,255,0.5)", fontSize: 14, textAlign: "center", maxWidth: 300 }}>{errorMsg}</p>
      </div>
    );
  }
  ...
  if (state === "ended") {
    return (
      <div style={fullCenter}>
        ...
        <p style={{ color: "rgba(255,255,255,0.5)", fontSize: 14 }}>Terima kasih telah menggunakan layanan telemedicine Puskesmas.</p>
      </div>
    );
  }
      ...
      <div style={fullCenter}>
        <div style={{
          ...
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <h1 style={{ color: "#fff", fontSize: 20, fontWeight: 700, margin: 0 }}>Konsultasi Video</h1>
          ...
          <div style={{
            ...
            <p style={{ color: "rgba(255,255,255,0.5)", fontSize: 11, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em", margin: "0 0 6px" }}>
              Jadwal Konsultasi
            </p>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <input
              ...
              onKeyDown={(e) => { if (e.key === "Enter" && displayName.trim()) void handleJoin(); }}
              style={{
  ...
  if (state === "connecting") {
    return (
      <div style={fullCenter}>
        <Loader2 size={36} style={{ color: "#D47A57", animation: "spin 1s linear infinite" }} />
        <p style={{ color: "rgba(255,255,255,0.5)", marginTop: 16, fontSize: 14 }}>Menghubungkan ke ruang konsultasi...</p>
      </div>
    );
  }
    ...
    <div style={{ height: "100dvh", display: "flex", flexDirection: "column", background: "#0a0a0a" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span style={{ color: "#fff", fontSize: 13, fontWeight: 600 }}>Konsultasi Berlangsung</span>
        </div>
        <span style={{ color: "rgba(255,255,255,0.4)", fontSize: 12 }}>Puskesmas Balowerti</span>
      </div>
      ...
      <div style={{
        ...
        <button onClick={() => void handleLeave()} style={{
          ...ctrlBtn(false),
