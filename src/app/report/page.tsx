...
export default function ReportPage() {
  ...
  const loadOutputFiles = useCallback(async () => {
    try {
      ...
      if (!res.ok) return;
      const data = await res.json() as { ok: boolean; files: OutputFile[]; outputDir: string };
      if (data.ok) {
  ...
  return (
    <div style={{ width: "100%", display: "flex", flexDirection: "column", alignItems: "flex-start" }}>
      <div className="page-header" style={{ maxWidth: 1400, width: "100%", display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
        <div>
        ...
        <div style={{
          ...
          ENGINE: TYPESCRIPT NATIVE<br />
          <span style={{ color: "#4ADE80" }}>● PYTHON NOT REQUIRED</span>
          {source && <><br />SRC: {source.toUpperCase()}</>}
        </div>
      </div>

      {/* Toolbar */}
      <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 28, maxWidth: 1400, width: "100%", flexWrap: "wrap" }}>
        <button
      ...
      {outputFiles.length > 0 && (
        <div style={{ maxWidth: 1400, width: "100%", marginBottom: 32 }}>
          <div style={{
            ...
            <button
              ...
              style={{
                ...
                letterSpacing: "0.1em", padding: "3px 10px", background: "none",
                border: "1px solid var(--line-base)", color: "var(--text-muted)",
                cursor: "pointer",
              }}
              title={outputDir}
            >
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                  <span style={{
                    ...
                    color: "var(--c-asesmen)", letterSpacing: "0.04em",
                  }}>
                    {f.name}
                  </span>
                  <span style={{
          ...
          <div style={{
            marginTop: 6, fontFamily: "var(--font-geist-mono), monospace",
            fontSize: 9, color: "var(--text-muted)", opacity: 0.5, letterSpacing: "0.08em",
          }}>
      ...
      <div style={{
        ...
        {summary.map((s) => (
          <div key={s.label} style={{
            ...
            <div className="v-label" style={{ marginBottom: 8 }}>{s.label}</div>
      ...
      {/* Filter tabs */}
      <div style={{ display: "flex", gap: 10, marginBottom: 24, maxWidth: 1400, width: "100%", flexWrap: "wrap" }}>
      ...
      <div style={{ maxWidth: 1400, width: "100%" }}>
        ...
        <div style={{
          ...
          borderBottom: "1px solid var(--line-base)", marginBottom: 4,
        }}>
          {["No. RM", "Pasien", "Tanggal", "Diagnosis", "ICD-X", "Dokter", "Status"].map((h) => (
            <span key={h} className="v-label">{h}</span>
          ))}
        </div>
        ...
        {filtered.map((row, i) => (
          ...
          >
            <span style={{
              fontFamily: "var(--font-geist-mono), monospace", fontSize: 10,
              color: "var(--c-asesmen)", letterSpacing: "0.05em",
            }}>{row.rm}</span>

            <span style={{
              fontFamily: "var(--font-geist-sans), sans-serif", fontSize: 15, color: "var(--text-main)",
            }}>{row.nama}</span>

            <span style={{
              fontFamily: "var(--font-geist-mono), monospace", fontSize: 10, color: "var(--text-muted)",
            }}>{row.tanggal}</span>
            ...
            <span style={{
              fontFamily: "var(--font-geist-sans), sans-serif", fontSize: 12, color: "var(--text-muted)",
            }}>{row.dokter}</span>
      ...
      {reportData.length > 0 && (
        <div style={{
          marginTop: 24, fontFamily: "var(--font-geist-mono), monospace",
          fontSize: 13, color: "var(--text-muted)", letterSpacing: "0.1em",
          opacity: 0.5,
        }}>
          MENAMPILKAN {filtered.length} DARI {reportData.length} KUNJUNGAN
          {source === "lb1-summary" ? " · DATA DARI LB1 ENGINE" : source === "default" ? " · DATA DEMO" : ""}
        </div>
      )}
    </div>
  );
