...
export default function EMRTransferPanel() {
  ...
  async function startTransfer() {
    ...
    try {
      const res = await fetch("/api/emr/transfer/run", {
        ...
        body: JSON.stringify({ payload, pelayananId: pelayananId || undefined }),
      });

      if (!res.ok) {
        const err = await res.json() as { error?: string };
        setProgressItems([{ step: "init", status: "failed", message: err.error || "Transfer gagal", timestamp: new Date().toISOString() }]);
      ...
      setProgressItems([{ step: "init", status: "failed", message: String(err), timestamp: new Date().toISOString() }]);
      setTransferState("error");
    }
  }
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: 12, display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        <button
          ...
          style={{
            ...
            color: keluhanUtama.trim() && transferState !== "running" ? "var(--c-asesmen, #a855f7)" : "var(--text-muted)",
            padding: "6px 12px",
            fontFamily: "inherit",
            fontSize: 11,
        ...
        <div style={{
          ...
          {progressItems.map((item, i) => (
            <div key={i} style={{
              display: "flex",
              gap: 8,
              color: item.status === "failed" ? "#ef4444" : item.status === "success" ? "#22c55e" : "var(--text-muted)",
              marginBottom: 2,
            }}>
      ...
      {/* History */}
      <div style={{ borderTop: "1px solid var(--line-base, #2a2a2a)", padding: "8px 12px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8, color: "var(--text-muted)" }}>
          <span>RIWAYAT TRANSFER</span>
          <button
            onClick={() => { void loadHistory(); }}
            style={{ background: "none", border: "none", color: "var(--text-muted)", cursor: "pointer", fontFamily: "inherit", fontSize: 11 }}
          >
            {historyLoading ? "..." : "↺ REFRESH"}
          </button>
        </div>
        {history.length === 0 ? (
          <div style={{ color: "var(--text-muted)", opacity: 0.4, fontSize: 11 }}>Belum ada riwayat.</div>
        ) : (
          history.map((entry) => (
            <div key={entry.id} style={{
              ...
              padding: "2px 0",
              borderBottom: "1px solid var(--line-base, #2a2a2a)",
              color: entry.state === "success" ? "#22c55e" : entry.state === "partial" ? "#f59e0b" : "#ef4444",
              fontSize: 11,
            }}>
              <span>{new Date(entry.timestamp).toLocaleString("id-ID")}</span>
              <span>{entry.state.toUpperCase()} · {entry.totalLatencyMs}ms</span>
            </div>
          ))
        )}
      </div>
      {socket && !socket.connected && (
        <div style={{ padding: "4px 12px", color: "#ef4444", fontSize: 10, borderTop: "1px solid var(--line-base)" }}>
