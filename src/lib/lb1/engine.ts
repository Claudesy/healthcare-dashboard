...
import { parseExportFile, readDiagnosisMapping, parseRegisterRujukan } from "./io";
import { normalizeRecords, aggregateForLb1, computeUnmappedDx, countDuplicates, countInvalidReasons, buildDisplayRows } from "./transform";
import { writeLb1Output, writeRegisLb1Output } from "./template-writer";
import { exportRmeVisitData } from "./rme-export";
import type { LB1RunResult, LB1Summary, LB1SummaryFile, EncounterRow, ParsedExport } from "./types";
...
function resolveLb1Paths(config: ReturnType<typeof loadLb1Config>): {
  ...
  return {
    ...
    outputDir: process.env.LB1_OUTPUT_DIR || resolveProjectPath(lb1.output_dir, getOutputDir()),
  ...
  try {
    ...
    if (shouldForceRmeExport && config?.rme?.base_url?.trim()) {
      try {
        ...
        console.warn("RPA export forced mode gagal:", rpaErr instanceof Error ? rpaErr.message : rpaErr);
      }
    }
    ...
    if (parsed.sourcePath && (parsed.sourcePath.endsWith(".xlsx") || parsed.sourcePath.endsWith(".xls"))) {
      try {
    ...
    const qcLines = [
      ...
      ...normalized.invalid.map((r) =>
        [r.source_row, r.visit_date, r.dx_code, r.sex, r.visit_type,
         r.age_year, r.age_month, r.age_day, `"${r.error_reason}"`].join(",")
      ),
    ];
    ...
    const lb1SummaryPath = path.join(outputDir, `SUMMARY_${year}_${String(month).padStart(2, "0")}.json`);
    await writeFile(
      lb1SummaryPath,
