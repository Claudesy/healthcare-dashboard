...
function loadStoredSession(sessionPath: string): object | null {
  ...
  try {
    ...
    if (Date.now() > expiresAt) {
      console.warn(`[rme-export] Session expired (expired: ${raw.expires_at}). Jalankan save-rme-session.ps1 untuk login ulang.`);
      return null;
    }
    const remainingMs = expiresAt - Date.now();
    const remainingHrs = (remainingMs / 3_600_000).toFixed(1);
    console.log(`[rme-export] Menggunakan stored session (valid ${remainingHrs} jam lagi, expires: ${raw.expires_at})`);
...
export async function exportRmeVisitData(
  ...
  if (!storedSession) {
    ...
    console.log("[rme-export] Tidak ada stored session — akan login via username/password (perlu no-CAPTCHA).");
  }
  ...
  for (let attempt = 1; attempt <= retryCount; attempt++) {
    ...
    try {
      ...
      const context = await browser.newContext(
        storedSession
          ? { acceptDownloads: true, storageState: storedSession as import("playwright").BrowserContextOptions["storageState"] }
