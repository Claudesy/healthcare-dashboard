...
function parseIcdRange(input: string): string[] | null {
  const rangeMatch = input.match(
    new RegExp(`^([A-Z])\\s*(\\d{2})(?:\\.(${ICD_SUBCODE_RE}))?\\s*(?:\\.\\.|-|S\\/D|SD|TO)\\s*([A-Z])?\\s*(\\d{2})(?:\\.(${ICD_SUBCODE_RE}))?$`),
  );
...
export function transformToIcd10_2010(
  ...
  const fromParen = normalizedInput.match(new RegExp(`\\(([A-Z][0-9]{2}(?:\\.(${ICD_SUBCODE_RE}))?)\\)\\s*$`));
  if (fromParen) {
  ...
  const regexMatches = normalizedInput.match(new RegExp(`\\b([A-Z][0-9]{2}(?:\\.(${ICD_SUBCODE_RE}))?)\\b`, "g")) ?? [];
