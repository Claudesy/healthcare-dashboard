...
import type { RawRow, NormalizedRow, InvalidRow, NormalizedData, DiagnosisMapping, AggregatedRow } from "./types";
import { SEX_MAP, VISIT_MAP, BUCKET_ORDER, VISIT_ORDER, SEX_ORDER } from "./constants";
...
export function normalizeRecords(
  ...
  for (const row of rows) {
    ...
    // Ekstrak ICD
    const dxTransform = transformToIcd10_2010(row.dx_code, { stripSubcode: stripIcdSubcode });
...
export function aggregateForLb1(
  ...
  for (const [key, count] of countMap) {
    ...
    if (agg) {
      agg.counts[`${bucket}_${visit}_${sex}`] = (agg.counts[`${bucket}_${visit}_${sex}`] ?? 0) + count;
    }
  }
