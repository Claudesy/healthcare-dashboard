...
function isRegisterFormat(raw: unknown[][]): boolean {
  ...
  const firstRow = (raw[0] as unknown[]).map((v) => String(v ?? "").trim().toUpperCase());
  // Format REGISTER punya header di row 1 dengan kolom khas
  return (
    firstRow.includes("NO.") || firstRow.includes("NO") ||
...
function parseRegisterFormat(raw: unknown[][], sourcePath: string): ParsedExport {
  ...
  if (colTanggal < 0 || colSex < 0 || colUmurThn < 0 || icdCols[0] < 0) {
    throw new Error("Format REGISTER: kolom wajib tidak ditemukan (Tanggal/JenisKelamin/UmurTahun/ICD-X1)");
  }
  ...
  for (let i = headerRowIdx + 1; i < raw.length; i++) {
    ...
    const umurThnRaw = String(r[colUmurThn] ?? "0").trim().replace(/\s*tahun\s*/i, "").trim();
    const umurBln   = colUmurBln >= 0 ? String(r[colUmurBln] ?? "0").trim().replace(/\s*bulan\s*/i, "").trim() : "0";
    const umurHari  = colUmurHari >= 0 ? String(r[colUmurHari] ?? "0").trim().replace(/\s*hari\s*/i, "").trim() : "0";
...
export function parseRegisterRujukan(filePath: string): RawRow[] {
  ...
  const raw = XLSX.utils.sheet_to_json<unknown[]>(rujukanSheet, { header: 1, defval: "" }) as unknown[][];
...
export function readDiagnosisMapping(filePath: string): DiagnosisMapping[] {
  ...
  if (idxRaw < 0 || idxIcd < 0 || idxNama < 0) {
    throw new Error("diagnosis_mapping.csv missing required columns: raw_dx, icdx10, jenis_penyakit");
  }
