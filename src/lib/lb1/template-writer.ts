...
import { BUCKET_ORDER, VISIT_ORDER, SEX_ORDER, lb1ColIndex, TOTAL_COL, START_DATA_ROW } from "./constants";
...
export function writeLb1Output(options: {
  ...
  for (let i = 0; i < aggregated.length; i++) {
    ...
    let totalKbL = 0, totalKbP = 0, totalKlL = 0, totalKlP = 0;
    ...
    // Kolom total
    ws[`${colIndexToName(TOTAL_COL["KB_L"])}${excelRow}`] = { v: totalKbL, t: "n" };
    ws[`${colIndexToName(TOTAL_COL["KB_P"])}${excelRow}`] = { v: totalKbP, t: "n" };
    ws[`${colIndexToName(TOTAL_COL["KL_L"])}${excelRow}`] = { v: totalKlL, t: "n" };
    ws[`${colIndexToName(TOTAL_COL["KL_P"])}${excelRow}`] = { v: totalKlP, t: "n" };
    ws[`${colIndexToName(TOTAL_COL["TOTAL_L"])}${excelRow}`] = { v: totalKbL + totalKlL, t: "n" };
    ws[`${colIndexToName(TOTAL_COL["TOTAL_P"])}${excelRow}`] = { v: totalKbP + totalKlP, t: "n" };
  }
  ...
  const outputBuffer = XLSX.write(wb, { bookType: "xlsx", bookSST: false, type: "buffer" }) as Buffer;
...
function writeRegisMetadata(ws: XLSX.WorkSheet, meta: RegisMetadata, bulanNama: string, bulanAngka: number, tahun: number): void {
