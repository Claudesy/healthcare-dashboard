...
interface JQObj { trigger: (e: string) => JQObj }
...
export async function fillTextField(
  ...
  if (!sel) {
    return { success: false, field: selectors, value, method: 'direct', error: 'Element not found' };
  }

  try {
    if (!forceOverride) {
      const current = await page.locator(sel).first().inputValue().catch(() => '');
    ...
    await page.evaluate((s: string) => {
      ...
      if ($el) { $el.trigger('input'); $el.trigger('change'); $el.trigger('blur'); }
    }, sel);

    return { success: true, field: sel, value, method: 'direct' };
  } catch (err) {
    return { success: false, field: sel, value, method: 'direct', error: String(err) };
  }
}
...
export async function fillNumberField(
  ...
  try {
    const current = await page.locator(sel).first().inputValue().catch(() => '');
    if (current.trim().length > 0) {
      return { success: true, field: sel, value: Number(current), method: 'direct' };
    }

    await page.locator(sel).first().fill(String(value));
    await page.evaluate((s: string) => {
      const $el = (window as JQWindow).$?.(s);
      if ($el) { $el.trigger('input'); $el.trigger('change'); }
    }, sel);
...
export async function fillSelect(
  ...
  const sel = await locateFirst(page, selectors);
  if (!sel) {
    return { success: false, field: selectors, value, method: 'select', error: 'Element not found' };
  }
  ...
  try {
    ...
    return { success: false, field: sel, value, method: 'select', error: String(err) };
  }
}
...
export async function fillCheckbox(
  ...
  const sel = await locateFirst(page, selectors);
  if (!sel) {
    return { success: false, field: selectors, value: checked, method: 'checkbox', error: 'Element not found' };
  }
  ...
  try {
    ...
    return { success: false, field: sel, value: checked, method: 'checkbox', error: String(err) };
  }
}
...
export async function activateCheckboxWithOnclick(
  ...
  const sel = await locateFirst(page, selectors);
  if (!sel) {
    return { success: false, field: selectors, value: shouldCheck, method: 'checkbox', error: 'Element not found' };
  }

  try {
    const isChecked = await page.locator(sel).first().isChecked();
    if (isChecked === shouldCheck) {
      return { success: true, field: sel, value: shouldCheck, method: 'checkbox' };
    }
    ...
    return { success: false, field: sel, value: shouldCheck, method: 'checkbox', error: String(err) };
  }
}
...
export async function fillRadio(
  ...
  const sel = await locateFirst(page, selectors);
  if (!sel) {
    return { success: false, field: selectors, value: _value, method: 'radio', error: 'Element not found' };
  }
  ...
  try {
    ...
    return { success: false, field: sel, value: _value, method: 'radio', error: String(err) };
  }
}
...
export async function fillAutocomplete(
  ...
  const sel = await locateFirst(page, selectors);
  if (!sel) {
    return { success: false, field: selectors, value, method: 'autocomplete', error: 'Element not found' };
  }
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      // 3. Wait for dropdown
      try {
        await page.waitForSelector(dropdownSelector, { timeout, state: 'visible' });
      } catch {
        if (requireDropdownSelection) {
          ...
          return { success: false, field: sel, value, method: 'autocomplete', error: 'Dropdown not appeared' };
        }
        // Fallback: value already typed, trigger change
        await page.evaluate((s: string) => {
          const $el = (window as JQWindow).$?.(s);
          if ($el) { $el.trigger('change'); $el.trigger('blur'); }
        }, sel);
      ...
      if (matchedIndex === -1) {
        ...
        return { success: false, field: sel, value, method: 'autocomplete', error: 'No matching item' };
      }
      ...
      // 6. Get final value
      const finalValue = await page.locator(sel).first().inputValue().catch(() => value);

      return { success: true, field: sel, value: finalValue, method: 'autocomplete' };
    } catch (err) {
      if (attempt === retries) {
        return { success: false, field: sel, value, method: 'autocomplete', error: String(err) };
      }
    }
  }

  return { success: false, field: sel, value, method: 'autocomplete', error: 'Max retries exceeded' };
}
...
export async function fillRangeSlider(
  ...
  try {
    ...
    if (!hiddenSel && !sliderSel) {
      return { success: false, field: hiddenSelector, value, method: 'direct', error: 'Neither hidden nor slider found' };
    }
    ...
    if (sliderSel) {
      ...
      await page.evaluate(([s, v]: [string, number]) => {
    ...
    return { success: false, field: hiddenSelector, value, method: 'direct', error: String(err) };
  }
}
  ...
  for (const field of fields) {
    ...
    switch (field.type) {
      ...
      case 'radio':
        ...
        break;
      case 'autocomplete':
        result = await fillAutocomplete(page, field.selector, String(field.value), field.autocompleteOptions);
        break;
      default:
