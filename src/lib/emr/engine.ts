...
export async function runEMRTransfer(
  ...
  const transferId = payload.options?.requestId || `emr-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  const ttlMs = config.sessionTtlMs ?? SESSION_TTL_MS;

  const emit = (step: EMRProgressEvent['step'], status: EMRProgressEvent['status'], message: string) => {
    emitEMRProgress({ transferId, step, status, message, timestamp: new Date().toISOString() });
  };
  ...
  const hasValidSession = isSessionValid(sessionStoragePath, config.username, ttlMs);
  ...
  try {
    ...
    result = await orchestrator.run(page, payload, {
      onProgress: (event) => {
        ...
        const status = event.state === 'running' ? 'running' : event.transferState === 'failed' ? 'failed' : 'success';
        emit(step, status, `Step ${step}: ${Object.values(event.steps).find((s) => s.step === step)?.state ?? 'running'}`);
      },
      onStepFinal: async (stepResult) => {
        // Navigate to next page between steps
        if (opts.pelayananId && (stepResult.state === 'success' || stepResult.state === 'partial')) {
          const nextStepMap: Record<string, string> = { anamnesa: 'diagnosa', diagnosa: 'resep' };
    ...
    emit('done', result.state === 'success' ? 'success' : 'failed',
    ...
    if (!result) {
      const steps = {
        anamnesa: { step: 'anamnesa' as const, state: 'failed' as const, attempt: 1, latencyMs: 0, successCount: 0, failedCount: 1, skippedCount: 0, message },
        diagnosa: { step: 'diagnosa' as const, state: 'skipped' as const, attempt: 0, latencyMs: 0, successCount: 0, failedCount: 0, skippedCount: 1 },
        resep: { step: 'resep' as const, state: 'skipped' as const, attempt: 0, latencyMs: 0, successCount: 0, failedCount: 0, skippedCount: 1 },
      };
      result = {
        runId: transferId,
        fingerprint: 'error',
