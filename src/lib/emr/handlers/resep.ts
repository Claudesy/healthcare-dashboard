...
const MEDICATION_NAME_ALIASES: Record<string, string[]> = {
  amoxicillin: ['amoksisilin'], amoksisilin: ['amoxicillin'],
  paracetamol: ['parasetamol'], parasetamol: ['paracetamol'],
  acetaminophen: ['asetaminofen', 'parasetamol'],
  aspirin: ['asam asetilsalisilat'], 'asam asetilsalisilat': ['aspirin'],
  diclofenac: ['diklofenak'], diklofenak: ['diclofenac'],
  'mefenamic acid': ['asam mefenamat'], 'asam mefenamat': ['mefenamic acid'],
  dexamethasone: ['deksametason'], deksametason: ['dexamethasone'],
  prednisone: ['prednison'], prednison: ['prednisone'],
  methylprednisolone: ['metilprednisolon'], metilprednisolon: ['methylprednisolone'],
  amlodipine: ['amlodipin'], amlodipin: ['amlodipine'],
  captopril: ['kaptopril'], kaptopril: ['captopril'],
  cetirizine: ['cetirizin', 'setirizin'], cetirizin: ['cetirizine', 'setirizin'], setirizin: ['cetirizine', 'cetirizin'],
  cefixime: ['sefiksim'], sefiksim: ['cefixime'],
  ceftriaxone: ['seftriakson'], seftriakson: ['ceftriaxone'],
  ciprofloxacin: ['siprofloksasin'], siprofloksasin: ['ciprofloxacin'],
  azithromycin: ['azitromisin'], azitromisin: ['azithromycin'],
  erythromycin: ['eritromisin'], eritromisin: ['erythromycin'],
  chlorpheniramine: ['klorfeniramin', 'ctm'], ctm: ['chlorpheniramine', 'klorfeniramin'],
  omeprazole: ['omeprazol'], omeprazol: ['omeprazole'],
  'vitamin c': ['asam askorbat'], 'asam askorbat': ['vitamin c'],
  'folic acid': ['asam folat'], 'asam folat': ['folic acid'],
  rifampicin: ['rifampisin'], rifampisin: ['rifampicin'],
  theophylline: ['teofilin'], teofilin: ['theophylline'],
  acyclovir: ['asiklovir'], asiklovir: ['acyclovir'],
};
...
function stripDose(v: string): string {
  return v.replace(/\b\d+\s*(mg|ml|mcg|g|gr|iu)\b/gi, ' ').replace(/\s+/g, ' ').trim();
}
...
function buildMedicationNameCandidates(rawName: string): string[] {
  ...
  const push = (v: string) => { if (v && v.length >= 2 && !candidates.includes(v)) candidates.push(v); };
...
function normalizeAturanPakai(raw: unknown): AturanPakai | null {
  ...
  const byLabel = ATURAN_PAKAI_ENTRIES.find(([, label]) => label.toLowerCase() === value.toLowerCase());
  return byLabel ? byLabel[0] : null;
}

async function checkStockAlert(page: Page): Promise<boolean> {
  const STOCK_ALERT_SELECTORS = [
    '.alert-danger', '.alert-warning', '.toast-message',
    '.noty_body', '.swal2-html-container', '.noty_message',
  ];
  ...
  for (const sel of STOCK_ALERT_SELECTORS) {
    try {
      ...
      const hasStock = norm.includes('stok obat') || (norm.includes('stok') && norm.includes('obat'));
      const hasFail = norm.includes('tidak mencukupi') || norm.includes('tidak cukup') || norm.includes('pilih obat lain');
...
async function verifyCommit(page: Page, rowIndex: number, expectedName: string): Promise<boolean> {
  ...
  for (let i = 0; i < maxAttempts; i++) {
    ...
    if (obatSel) {
      const val = await page.locator(obatSel).first().inputValue().catch(() => '');
      if (val.trim().length > 0) {
        const normActual = normalizeLabel(val);
        const normExpected = normalizeLabel(stripDose(removeParen(expectedName)));
        if (normActual.includes(normExpected.split(' ')[0] || '') || normExpected.includes(normActual.split(' ')[0] || '')) {
          return true;
        }
      }
    }
  ...
  if (payload.ajax.ruangan) {
    ...
    if (ruanganSel) {
      await fillAutocomplete(page, ruanganSel, payload.ajax.ruangan, {
        timeout: 1000, retries: 2, allowFirstItemFallback: true,
      });
    }
  }
  ...
  if (payload.ajax.dokter) {
    ...
    if (dokterSel) {
      await fillAutocomplete(page, dokterSel, payload.ajax.dokter, {
        timeout: 1500, retries: 2, allowFirstItemFallback: true,
      });
    }
  }
...
async function fillMedicationRow(
...
): Promise<{ state: ResepRowState; reasonCode?: ResepRuntimeReasonCode; error?: string }> {
  const selectors = getResepRowSelectors(rowIndex);
  ...
  if (!medicationFilled) {
    return { state: 'failed', reasonCode: 'ROW_NOT_READY', error: `Medication not found: ${med.nama_obat}` };
  }
  ...
  if (stockFailed) {
    return { state: 'failed', reasonCode: 'STOCK_INSUFFICIENT', error: `Stok tidak mencukupi: ${med.nama_obat}` };
  }
  ...
  if (signaSel) {
    await fillAutocomplete(page, signaSel, signa, {
      timeout: 800, retries: 2, allowFirstItemFallback: true,
    });
...
export interface ResepFillResult {
  success: boolean;
  filledRows: number;
  failedRows: Array<{ index: number; nama_obat: string; error?: string; reasonCode?: ResepRuntimeReasonCode }>;
  error?: string;
}
...
export async function fillResep(page: Page, payload: ResepFillPayload): Promise<ResepFillResult> {
  ...
  for (let i = 0; i < payload.medications.length; i++) {
    ...
    if (i > 0) {
      ...
      if (!added) {
        failedRows.push({ index: i, nama_obat: med.nama_obat, error: 'Add row button not found' });
        continue;
      }
