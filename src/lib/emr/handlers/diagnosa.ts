...
async function fillSelectWithFallbacks(
  ...
  for (const sel of selectors) {
    try {
      ...
      const optionValues = await page.evaluate((s: string): Array<{ value: string; text: string }> => {
...
export async function fillDiagnosa(
  ...
  if (payload.prognosa) {
    ...
    const ok = await fillSelectWithFallbacks(page, prognosaSelectors, [prognosaValue, payload.prognosa]);
