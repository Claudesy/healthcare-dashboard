...
async function fillAlergiSection(page: Page, alergi: AnamnesaFillPayload['alergi']): Promise<void> {
  ...
  const alergiGroups = [
    { key: 'obat', items: alergi.obat, nameSel: 'input[name*="alergi_obat"]', areaSel: 'textarea[name*="alergi_obat"]' },
    { key: 'makanan', items: alergi.makanan, nameSel: 'input[name*="alergi_makanan"]', areaSel: 'textarea[name*="alergi_makanan"]' },
    { key: 'udara', items: alergi.udara, nameSel: 'input[name*="alergi_udara"]', areaSel: 'textarea[name*="alergi_udara"]' },
    { key: 'lainnya', items: alergi.lainnya, nameSel: 'input[name*="alergi_lainnya"]', areaSel: 'textarea[name*="alergi_lainnya"]' },
  ];
...
export async function fillAnamnesa(
  ...
  if (payload.vital_signs) {
    ...
    const vsFields: FieldMapping[] = [
      {
        selector: 'input[name="PeriksaFisik[sistole]"], input[name*="[sistole]"], input[id*="sistole"]',
        value: vs.tekanan_darah_sistolik,
        type: 'number',
      },
      {
        selector: 'input[name="PeriksaFisik[diastole]"], input[name*="[diastole]"], input[id*="diastole"]',
        value: vs.tekanan_darah_diastolik,
        type: 'number',
      },
      {
        selector: 'input[name="PeriksaFisik[detak_nadi]"], input[name*="[detak_nadi]"], input[id*="nadi"]',
        value: vs.nadi,
        type: 'number',
      },
      {
  ...
  if (payload.periksa_fisik) {
    ...
    const pfFields: FieldMapping[] = [
      ...
      {
        selector: 'input[name*="[gcs_verbal]"], select[name*="[gcs_verbal]"], input[id*="gcs_verbal"]',
        value: pf.gcs_respon_verbal,
        type: 'select',
      },
      {
        selector: 'input[name*="[gcs_motorik]"], select[name*="[gcs_motorik]"], input[id*="gcs_motorik"]',
    ...
    for (const af of activityFields) {
      ...
      if (found) {
        const tagName = await page.locator(found).first().evaluate((el) => el.tagName.toLowerCase());
        if (tagName === 'select') {
          await fillSelect(page, found, af.val);
        } else {
          await page.locator(found).first().click().catch(() => null);
        }
      }
    }
  }
  ...
  if (payload.assesmen_nyeri) {
    ...
    if (merasakan_nyeri === '1') {
      ...
      const nyeriSel = 'input[name*="[merasakan_nyeri]"][value="1"], input[id*="merasakan_nyeri_ya"]';
      const found = await locateFirst(page, nyeriSel);
      if (found) await page.locator(found).first().click().catch(() => null);
  ...
  if (payload.keadaan_fisik) {
    ...
    const areas = Object.entries(kf) as [string, { inspeksi: string; palpasi?: string } | undefined][];
  ...
  if (payload.riwayat_penyakit) {
    ...
    const rpFields: FieldMapping[] = [
      {
        selector: 'textarea[name*="[rps]"], textarea[id*="rps"], textarea[name*="riwayat_sekarang"]',
        value: rp.sekarang,
        type: 'textarea',
        forceOverride: true,
      },
      ...
      {
        selector: 'textarea[name*="[rpk]"], textarea[id*="rpk"], textarea[name*="riwayat_keluarga"]',
        value: rp.keluarga,
        type: 'textarea',
        forceOverride: true,
      },
  ...
  if (payload.status_psikososial) {
    ...
    const spFields: FieldMapping[] = [
      {
        selector: 'select[name*="[alat_bantu_aktrifitas]"], input[name*="[alat_bantu]"][value="${sp.alat_bantu_aktrifitas}"]',
