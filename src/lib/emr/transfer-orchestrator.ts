...
function createStepResult(step: RMETransferStepStatus): RMETransferStepResult {
  return { step, state: 'pending', attempt: 0, latencyMs: 0, successCount: 0, failedCount: 0, skippedCount: 0 };
}
...
): Promise<{ success: boolean; filledFields: string[]; failedFields: string[] }> {
  switch (step) {
    case 'anamnesa':
...
function normalizeHandlerResult(raw: { success: boolean; filledFields: string[]; failedFields: string[] }) {
  return {
...
export class RMETransferOrchestrator {
  ...
  async run(
    ...
    const dedupeWindowMs = transferPayload.options?.idempotencyWindowMs || options.dedupeWindowMs || DEFAULT_DEDUPE_WINDOW_MS;
    const fingerprint = hashString(stableStringify({
      ...
      resep: transferPayload.resep ? {
        ...
        static: { ...transferPayload.resep.static, no_resep: '__stable__' },
      } : null,
    }));
    const runId = transferPayload.options?.requestId || `rme-${startedMs}-${Math.random().toString(36).slice(2, 8)}`;
    ...
    const isDuplicate = !transferPayload.options?.forceRun && latest && startedMs - latest.timestampMs < dedupeWindowMs;

    if (isDuplicate) {
      reasonCodes.add('DUPLICATE_SUPPRESSED');
      return {
        runId, fingerprint, state: 'failed', startedAt, completedAt: nowIso(),
        totalLatencyMs: 0, reasonCodes: Array.from(reasonCodes), steps,
      };
    }

    this.activeRuns.set(runId, { fingerprint, cancelled: false });
    this.emitProgress(options.onProgress, { runId, state: 'running', transferState: 'partial', steps, reasonCodes: Array.from(reasonCodes) });

    const stepOrder = resolveStepOrder(transferPayload.options?.startFromStep, transferPayload.options?.onlyStep);
    const timeoutMs = { ...DEFAULT_TIMEOUT_MS, ...(options.timeoutMs || {}) };
    const retryByStep = { ...DEFAULT_RETRY_BY_STEP, ...(options.retryByStep || {}) };
    const retryDelayMs = options.retryDelayMs || DEFAULT_RETRY_DELAY_MS;

    for (const step of stepOrder) {
      if (this.isCancelled(runId)) {
        steps[step] = { ...steps[step], state: 'cancelled', reasonCode: 'USER_CANCELLED', errorClass: 'fatal', message: 'Transfer dibatalkan' };
      ...
      if (!payload) {
        const rc: RMETransferReasonCode = step === 'diagnosa' ? 'DIAGNOSA_PAYLOAD_EMPTY' : 'RESEP_PAYLOAD_EMPTY';
        steps[step] = { ...steps[step], state: 'skipped', reasonCode: rc, errorClass: 'recoverable', message: `Payload ${step} kosong` };
        reasonCodes.add(rc);
        this.emitProgress(options.onProgress, { runId, state: 'running', transferState: resolveResultState(steps), activeStep: step, steps, reasonCodes: Array.from(reasonCodes) });
        continue;
      }
      ...
      for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
        if (this.isCancelled(runId)) {
          steps[step] = { ...steps[step], state: 'cancelled', attempt, reasonCode: 'USER_CANCELLED', errorClass: 'fatal' };
          reasonCodes.add('USER_CANCELLED');
          break;
        }
        ...
        try {
          ...
          if (normalized.successCount > 0 && normalized.failedCount === 0) {
            stepResult = { step, state: 'success', attempt, latencyMs, successCount: normalized.successCount, failedCount: 0, skippedCount: 0 };
            break;
          }
          if (normalized.successCount > 0) {
            stepResult = { step, state: 'partial', attempt, latencyMs, successCount: normalized.successCount, failedCount: normalized.failedCount, skippedCount: 0, reasonCode: 'NO_FIELDS_FILLED', errorClass: 'recoverable', message: normalized.errors[0] };
            break;
          }

          if (attempt > maxRetries) {
            stepResult = { step, state: 'failed', attempt, latencyMs, successCount: 0, failedCount: Math.max(1, normalized.failedCount), skippedCount: 0, reasonCode: 'RETRY_EXHAUSTED', errorClass: 'recoverable', message: normalized.errors[0] || 'Step gagal' };
          ...
          if (attempt > maxRetries) {
            stepResult = { step, state: 'failed', attempt, latencyMs, successCount: 0, failedCount: 1, skippedCount: 0, reasonCode: isTimeout ? 'STEP_TIMEOUT' : 'RETRY_EXHAUSTED', errorClass: 'recoverable', message };
      ...
      if (!stepResult) {
        stepResult = { ...steps[step], state: 'failed', reasonCode: 'UNKNOWN_STEP_FAILURE', errorClass: 'fatal', message: 'Step gagal' };
    ...
    const finalResult: RMETransferResult = { runId, fingerprint, state: transferState, startedAt, completedAt, totalLatencyMs, reasonCodes: Array.from(reasonCodes), steps };
    this.emitProgress(options.onProgress, { runId, state: transferState === 'cancelled' ? 'cancelled' : 'completed', transferState, steps, reasonCodes: finalResult.reasonCodes });
